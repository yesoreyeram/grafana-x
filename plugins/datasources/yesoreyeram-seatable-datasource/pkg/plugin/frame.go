...
// recordsToFrame converts a slice of SeaTable rows (flat JSON maps keyed by
// column name, optionally including the `_id`/`_ctime`/`_mtime` identity columns)
// into a single wide data.Frame conforming to the data plane "table" contract.
//
// Row order is preserved exactly as returned by SeaTable, so the query's sort /
// view order is honoured. Time fields are moved to the front of the columns so
...
// The synthetic identity columns (_id, _ctime, _mtime) lead, followed by the
// union of user column names sorted alphabetically.
func orderedColumns(records []map[string]any) []string {
	seen := map[string]bool{}
	var ordered []string

	// Identity columns first, when present in any record.
	for _, id := range []string{"_id", "_ctime", "_mtime"} {
...
func toString(v any) (string, bool) {
	...
	default:
		// Objects/arrays (multiple-select, collaborator, link, image, file
		// columns) are serialised as JSON so nested data is still visible.
...
// SeaTable date / ctime / mtime values are ISO 8601 / RFC 3339, e.g.
// "2025-09-15T10:57:19.106+02:00", "2025-09-18T09:52:00+00:00", "2024-01-15"
// or the SQL time-constant form "2020-09-08 00:11:23".
