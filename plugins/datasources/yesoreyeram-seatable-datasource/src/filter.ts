...
// Filter model
//
// The structured filter tree is compiled into a parameterized SeaTable SQL
// WHERE clause on the backend. The frontend only describes conditions/groups;
// it does not build the SQL itself. `field` is the SeaTable column *name*.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Operator catalog (column-type aware)
// ---------------------------------------------------------------------------
...
/** Logical categories a SeaTable column type can map to for operator selection. */
export type FieldCategory = 'text' | 'number' | 'boolean' | 'date';

// Map SeaTable column types to a logical category. Unknown types default to text.
...
/** Returns the operator definitions valid for a given SeaTable column type. */
...
//
// The SQL is built on the backend from the structured tree. Because backend
