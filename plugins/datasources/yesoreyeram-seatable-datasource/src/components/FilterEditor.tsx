...
// the column / operator / value columns line up.
...
export function FilterEditor({ group, columns, disabled, onChange }: FilterEditorProps) {
  return <GroupEditor group={group} columns={columns} disabled={disabled} depth={0} onChange={onChange} />;
}
...
function GroupEditor({ group, columns, disabled, depth, onChange }: GroupEditorProps) {
...
function ConditionRow({ condition, columns, disabled, onChange, onRemove }: ConditionRowProps) {
  // Conditions reference columns by name (the backend compiles SQL using names).
  const fieldOptions: Array<SelectableValue<string>> = columns.map((c) => ({
    label: c.name,
    value: c.name,
    description: c.type,
  }));

  const selectedColType = columns.find((c) => c.name === condition.field)?.type;
  const opOptions = operatorOptions(selectedColType);
  const arity = operatorArity(condition.op);

  const onFieldChange = (v: SelectableValue<string> | null) => {
    const field = v?.value ?? '';
    const newType = columns.find((c) => c.name === field)?.type;
    const validOps = operatorOptions(newType);
    // Keep current op if still valid for the new column type, else default to first.
