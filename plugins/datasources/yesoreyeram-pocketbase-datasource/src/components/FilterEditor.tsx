...
// the field / operator / value columns line up.
...
function ConditionRow({ condition, fields, disabled, onChange, onRemove }: ConditionRowProps) {
  ...
  const selectedType = fields.find((f) => f.name === condition.attribute)?.type;
  ...
  const onFieldChange = (v: SelectableValue<string> | null) => {
    ...
    const newType = fields.find((f) => f.name === attribute)?.type;
  ...
  return (
    <>
      <Select<string>
        ...
        value={
          condition.attribute
            ? (fieldOptions.find((o) => o.value === condition.attribute) ?? {
