...
func (d *Datasource) query(ctx context.Context, query backend.DataQuery) backend.DataResponse {
	...
	switch qm.QueryType {
	case QueryTypeRows, "":
		rows, err := d.client.ListRows(ctx, qm)
		if err != nil {
			log.DefaultLogger.Error("coda query failed", "refID", query.RefID, "error", err)
			return backend.ErrDataResponse(backend.StatusInternal, "query failed: "+err.Error())
		}
		frame := rowsToFrame(query.RefID, rows)
		return backend.DataResponse{Frames: []*data.Frame{frame}}
	case QueryTypeCount:
		count, err := d.client.CountRows(ctx, qm)
		if err != nil {
			log.DefaultLogger.Error("coda count query failed", "refID", query.RefID, "error", err)
			return backend.ErrDataResponse(backend.StatusInternal, "query failed: "+err.Error())
		}
...
func (d *Datasource) CheckHealth(ctx context.Context, _ *backend.CheckHealthRequest) (*backend.CheckHealthResult, error) {
	if strings.TrimSpace(d.settings.apiToken) == "" {
		return &backend.CheckHealthResult{
			...
			Message: "Coda API token is not configured (re-enter it; saved secrets are write-only)",
		}, nil
	}
	...
	if err := d.client.Ping(ctx); err != nil {
		return &backend.CheckHealthResult{
			Status:  backend.HealthStatusError,
			Message: "Failed to connect to Coda: " + err.Error(),
		}, nil
	}
...
func (d *Datasource) handleColumns(rw http.ResponseWriter, r *http.Request) {
	...
	columns, err := d.client.ListColumns(r.Context(), docID, tableID)
	if err != nil {
		writeJSON(rw, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
