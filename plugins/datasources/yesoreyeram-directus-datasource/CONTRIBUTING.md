...
Thanks for your interest in improving the Grafana Directus data source plugin.
...
- A Directus instance with an [API token](https://docs.directus.io/reference/authentication.html#static-tokens)
...
| `pkg/` | Backend (Go): Directus client, query handling, filter compiler, data-frame conversion |
...
Yarn 4 monorepo under `plugins/datasources/yesoreyeram-directus-datasource`. Install dependencies
...
cd grafana-x
yarn install
cd plugins/datasources/yesoreyeram-directus-datasource
```
...
# Backend → dist/gpx_directus_<os>_<arch>
...
# grafana.ini / env
[plugins]
allow_loading_unsigned_plugins = yesoreyeram-directus-datasource
```

Symlink or copy `dist/` into Grafana's plugins directory as
`yesoreyeram-directus-datasource`, then restart Grafana.

### Run the full local stack (recommended)

Directus is self-hosted, so the stack is just Grafana provisioned against your
Directus instance:
...
```
QueryEditor (React)
  → builds DirectusQuery { queryType, collectionId, fields, sort, filterTree, limit, offset, search }
  → applyTemplateVariables() interpolates filter values, collection, fields, search
  → backend QueryData (pkg/plugin/datasource.go)
      ...
      → client.ListRecords / client.CountRecords (pkg/plugin/client.go)
          → BuildFilter compiles the filter tree → Directus JSON filter object (pkg/plugin/filter.go)
          → Directus REST API, auto-paginated via offset
      → recordsToFrame / countToFrame (pkg/plugin/frame.go) → Grafana data.Frame
```

### Things that must not regress

- **Filters are compiled server-side** from the JSON `filterTree` into a Directus
  JSON filter object.
- **Row order is preserved** in `recordsToFrame` — Directus's returned order
...
- Records are listed at `/items/{collection}`, paged via **offset+limit**
  (not cursor). Params: `fields`, `filter` (JSON object), `sort` (comma-separated,
  prefix with `-` for descending), `limit`, `offset`, `search`.
- Schema (collections/fields) comes from `/collections` and `/fields/{collection}`.
- Count uses `GET /items/{collection}?meta=total_count&limit=0`. Falls back to
  counting the `data` array when `meta` is unavailable.
...
curl -s "http://localhost:3000/api/datasources/uid/<uid>/resources/fields?collectionId=<name>"

# run a query
curl -s -X POST http://localhost:3000/api/ds/query -H 'Content-Type: application/json' \
  -d '{"queries":[{"refId":"A","datasource":{"uid":"<uid>","type":"yesoreyeram-directus-datasource"},"queryType":"records","collectionId":"<name>"}],"from":"now-2y","to":"now"}'
```
