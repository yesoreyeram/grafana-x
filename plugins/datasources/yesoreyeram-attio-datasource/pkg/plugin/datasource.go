...
// Datasource is the Attio backend data source instance. One instance exists per
...
func (d *Datasource) query(ctx context.Context, query backend.DataQuery) backend.DataResponse {
	...
	case QueryTypeRecords, "":
		records, err := d.client.QueryRecords(ctx, qm)
		if err != nil {
			log.DefaultLogger.Error("attio query failed", "refID", query.RefID, "error", err)
	...
	case QueryTypeCount:
		...
		if err != nil {
			log.DefaultLogger.Error("attio count query failed", "refID", query.RefID, "error", err)
			return backend.ErrDataResponse(backend.StatusInternal, "query failed: "+err.Error())
		}
...
func (d *Datasource) CheckHealth(ctx context.Context, _ *backend.CheckHealthRequest) (*backend.CheckHealthResult, error) {
	if d.settings.BaseURL == "" {
		return &backend.CheckHealthResult{
			...
			Message: "Attio base URL is not configured",
		}, nil
	}
	if strings.TrimSpace(d.settings.apiToken) == "" {
		return &backend.CheckHealthResult{
			Status:  backend.HealthStatusError,
			Message: "Attio API token is not configured (re-enter it; saved secrets are write-only)",
		}, nil
	}
	...
	if err := d.client.Ping(ctx); err != nil {
		return &backend.CheckHealthResult{
			Status:  backend.HealthStatusError,
			Message: "Failed to connect to Attio: " + err.Error(),
		}, nil
	}

	return &backend.CheckHealthResult{
		Status:  backend.HealthStatusOk,
		Message: "Successfully connected to Attio",
	}, nil
}

// CallResource routes resource calls (used by the frontend to fetch object and
// attribute lists).
...
func (d *Datasource) newResourceMux() *http.ServeMux {
	...
	mux.HandleFunc("/objects", d.handleObjects)
	mux.HandleFunc("/attributes", d.handleAttributes)
	return mux
}

// handleObjects serves GET /objects returning the workspace's objects.
func (d *Datasource) handleObjects(rw http.ResponseWriter, r *http.Request) {
	objects, err := d.client.ListObjects(r.Context())
	if err != nil {
		writeJSON(rw, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
	writeJSON(rw, http.StatusOK, map[string]any{"objects": objects})
}

// handleAttributes serves GET /attributes?objectId=... returning the object's
// attributes.
func (d *Datasource) handleAttributes(rw http.ResponseWriter, r *http.Request) {
	objectID := r.URL.Query().Get("objectId")
	attributes, err := d.client.ListAttributes(r.Context(), objectID)
	if err != nil {
		writeJSON(rw, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
