...
function ConditionRow({ condition, fields, disabled, onChange, onRemove }: ConditionRowProps) {
  // Conditions reference fields by NAME (the backend queries with fieldKeyType=name).
  const fieldOptions: Array<SelectableValue<string>> = fields.map((c) => ({
    label: c.name,
    value: c.name,
    description: c.type,
  }));

  const selectedColType = fields.find((c) => c.name === condition.field)?.type;
  ...
  const onFieldChange = (v: SelectableValue<string> | null) => {
    const field = v?.value ?? '';
    const newType = fields.find((c) => c.name === field)?.type;
    const category = categoryForType(newType);
    const validOps = operatorsForType(newType);
    const op = validOps.some((o) => o.value === condition.op) ? condition.op : (validOps[0]?.value ?? 'is');
  ...
  return (
    <>
      ...
      <Select<string>
        ...
        onChange={(v) => onChange({ ...condition, op: v?.value ?? 'is' })}
      ...
      {arity === 'none' ? (
        ...
        <Input
          ...
          placeholder={arity === 'list' ? 'Comma-separated values' : 'Value'}
