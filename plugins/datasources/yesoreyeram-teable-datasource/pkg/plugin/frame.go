...
// The synthetic identity columns (_id, _createdTime, _lastModifiedTime) lead,
// followed by the union of user field names sorted alphabetically. Time columns
// are later moved ahead of the rest by orderTimeFirst.
func orderedColumns(records []map[string]any) []string {
	seen := map[string]bool{}
	var ordered []string

	// Identity columns first, when present in any record.
	for _, id := range []string{"_id", "_createdTime", "_lastModifiedTime"} {
