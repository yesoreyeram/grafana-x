...
Thanks for your interest in improving the Grafana Baserow data source plugin.
...
| `pkg/` | Backend (Go): Baserow client, query handling, filter compiler, data-frame conversion |
...
Yarn 4 monorepo under `plugins/datasources/yesoreyeram-baserow-datasource`. Install dependencies
...
cd grafana-x
yarn install
cd plugins/datasources/yesoreyeram-baserow-datasource
```
...
# Backend → dist/gpx_baserow_<os>_<arch>
...
# grafana.ini / env
[plugins]
allow_loading_unsigned_plugins = yesoreyeram-baserow-datasource
```

Symlink or copy `dist/` into Grafana's plugins directory as
`yesoreyeram-baserow-datasource`, then restart Grafana.
...
- **Baserow** at http://localhost:8980 — **empty** (no sample-data seeding;
  first-boot template sync is disabled so it starts quickly)
- **Grafana** at http://localhost:3000 with **anonymous admin** (no login needed)

Create a database/table and a database token (or use email/password) in the
Baserow UI, then auto-provision the datasource by exporting credentials:
...
Omit the variables to add the datasource manually in the Grafana UI.
...
```
QueryEditor (React)
  → builds BaserowQuery { queryType, tableId, viewId, fields, sort, filterTree, limit }
  ...
  → backend QueryData (pkg/plugin/datasource.go)
      ...
      → client.ListRecords / client.CountRecords (pkg/plugin/client.go)
          → BuildFilters compiles the filter tree → Baserow `filters` JSON (pkg/plugin/filter.go)
          → Baserow REST API, auto-paginated, user_field_names=true
      → recordsToFrame / countToFrame (pkg/plugin/frame.go) → Grafana data.Frame
```

### Things that must not regress

- **Filters are compiled server-side** from the JSON `filterTree` into the Baserow
  `filters` tree (`{filter_type, filters, groups}`).
- **Row order is preserved** in `recordsToFrame` — Baserow's returned order already
  honours the query `order_by`. Do not re-sort rows.
- **Data-plane compliance**: records → `FrameTypeTable`; count →
  `FrameTypeNumericWide`. Date/DateTime columns become UTC `*time.Time` fields.
- **Secrets stay on the server**: the database token / password are `secureJsonData`;
  never log them or send them to the browser.

### Baserow API quirks

- **Two auth modes**: a **database token** (`Authorization: Token <token>`, scoped
  to one database — hence the Database ID), or **email + password** exchanged for a
  **JWT** (`Authorization: JWT <jwt>`, auto-refreshed on 401, can list every
  database). Password mode powers the `/databases` resource and the editor's
  Database picker.
- Rows are listed with `user_field_names=true`, paged via `page`+`size` (max 200),
  sorted via `order_by`, projected via `include`, filtered via the `filters` tree.
- There is **no group-by / general aggregation** endpoint and **no dedicated count
  endpoint** — Count reads `count` from a `size=1` row request. Aggregate in
...
curl -s -X POST http://localhost:3000/api/ds/query -H 'Content-Type: application/json' \
  -d '{"queries":[{"refId":"A","datasource":{"uid":"<uid>","type":"yesoreyeram-baserow-datasource"},"queryType":"records","tableId":"<id>"}],"from":"now-2y","to":"now"}'
```
