...
Thanks for your interest in improving the Grafana Todoist data source plugin.
...
| `pkg/` | Backend (Go): Todoist v1 API client, query handling, task flattener, data-frame conversion |
...
Yarn 4 monorepo under `plugins/datasources/yesoreyeram-todoist-datasource`.
...
cd grafana-x
yarn install
cd plugins/datasources/yesoreyeram-todoist-datasource
```
...
# Backend → dist/gpx_todoist_<os>_<arch>
...
login needed) and the Todoist data source pre-configured from `TODOIST_API_TOKEN`.
...
```
QueryEditor (React)
  → builds TodoistQuery { queryType, projectId/sectionId/label/parentId,
    filter, lang, limit }
  → applyTemplateVariables() interpolates the scope inputs + filter
  → backend QueryData (pkg/plugin/datasource.go)
      → LoadQuery parses the query
      → tasks:  client.ListRecords  → taskPathParams routes /tasks vs /tasks/filter
      → count:  client.CountRecords → paginate + count (no flatten)
          → cursor-paginate (limit/cursor + next_cursor)
          → flattenItem reduces due/deadline/duration/labels (pkg/plugin/frame.go)
      → recordsToFrame / countToFrame (pkg/plugin/frame.go) → Grafana data.Frame
```

### Things that must not regress

- **v1 unified API**: base `https://api.todoist.com/api/v1`. Never revert to REST
  v2 — its `/tasks` is un-paginated and ignores `offset`/`limit`.
- **Cursor pagination**: list endpoints return `{results, next_cursor}`; follow
  `next_cursor` until null/empty. Resource lists (`/projects`, `/sections`,
  `/labels`) paginate too.
- **Filter is a separate endpoint** (`/tasks/filter?query=`); the `filter`/`lang`
  params were removed from `/tasks` and the two cannot be combined.
- **`label` filters by name**, not ID.
- **Auth is Bearer**: token sent as `Authorization: Bearer`. Secret is `apiToken`.
- **v1 field renames** + `due` having no `datetime` field — see AGENTS.md.
- **Active tasks only**; no native count endpoint (paginate + count).
...
- **Secrets stay on the server**: never log the token or send it to the browser.
...
- Go HTTP behavior is tested with `net/http/httptest`; tests are table-driven with
  `testify`.
- For UI, prefer testing the extracted helpers and the data source class.
...
- **TypeScript**: use only stable `@grafana/ui` components — the plugin targets
  ...
  Grafana; use `Select` / `Input` / `InlineField`.
