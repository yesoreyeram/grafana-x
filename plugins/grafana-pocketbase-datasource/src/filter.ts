...
// Filter model
//
// The structured filter tree is compiled into a PocketBase filter expression on
...
export interface FilterCondition {
  kind: 'condition';
  /** PocketBase field name. */
  attribute: string;
  /** Operator (see OPERATORS). Mapped to PocketBase operators server-side. */
  op: string;
...
/** Logical categories a PocketBase field type can map to for operator selection. */
export type AttributeCategory = 'text' | 'number' | 'boolean' | 'datetime';

// Map PocketBase field types to a logical category. Unknown types default to text.
...
/** Returns the operator definitions valid for a given PocketBase field type. */
...
//
// The filter expression is built on the backend from the structured tree.
// Because backend data sources interpolate the query on the frontend before it
// is sent, we interpolate each condition value here so the backend receives
// concrete values.
// ---------------------------------------------------------------------------
