...
// recordsToFrame converts a slice of Airtable records (flat JSON maps, including
// the synthetic _id/_createdTime columns) into a single wide data.Frame
// conforming to the data plane "table" contract.
//
// Row order is preserved exactly as returned by Airtable, so the query's sort
// (or the view's order) is honoured. Time fields are moved to the front of the
// columns so that time-series and Explore consumers detect the time dimension,
// but this does NOT change the order of the rows themselves.
...
// The synthetic identity columns (_id, _createdTime) lead, followed by the union
// of user field names sorted alphabetically.
func orderedColumns(records []map[string]any) []string {
	seen := map[string]bool{}
	var ordered []string

	// Identity columns first, when present in any record.
	for _, id := range []string{"_id", "_createdTime"} {
...
func toString(v any) (string, bool) {
	...
	default:
		// Objects/arrays (attachments, linked records, collaborators,
		// multi-selects) are serialised as JSON so nested data is still visible.
...
var timeLayouts = []string{
	time.RFC3339Nano,
	time.RFC3339,
	// Airtable date/dateTime fields render as ISO 8601, e.g.
	// "2026-06-15T09:30:55.000Z" or date-only "2026-06-15".
