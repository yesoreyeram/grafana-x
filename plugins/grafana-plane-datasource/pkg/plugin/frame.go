...
// dateKeys lists the Plane fields whose values are ISO-8601 / RFC3339 date-time
// strings (or date-only strings) and should be presented as time columns. Plane
// returns timestamps as RFC3339 strings (e.g. "2024-01-01T00:00:00Z") and plain
// dates as "2024-01-01".
...
// recordsToFrame converts a slice of flattened Plane records (arbitrary JSON
// objects keyed by field name) into a single wide data.Frame conforming to the
// data plane "table" contract.
//
// Row order is preserved exactly as returned by Plane, so the query's ordering
...
// specific type that fits every value. Known Plane date columns (RFC3339 / date
// strings) are treated as time. Mixed columns fall back to string.
...
// toColumnTime interprets a value as a time for the given column. For known
// Plane date columns the value is an RFC3339 / date-only string; non-date
// columns are never treated as time (so plain numbers stay numeric).
...
// timeLayouts lists the formats recognised when parsing Plane date columns
// (RFC3339 timestamps and plain dates) and time values from raw queries.
...
// ---------------------------------------------------------------------------
// Plane entity flattening
//
// A Plane entity (work item, project, state, label, cycle, module, member) is a
// nested JSON object. flattenEntity reduces each nested object/array to a scalar
// (or compact string) so records can flow through the same type-inference /
// frame builder as a flat table. Specific known relations are mapped to their
// most useful representation; other nested values fall back to a generic
// flattener.
// ---------------------------------------------------------------------------

// flattenEntity converts a single Plane entity object into a flat record keyed
// by field name. Nested relations are reduced to readable scalars.
func flattenEntity(raw json.RawMessage) map[string]any {
	...
	for key, value := range obj {
		switch key {
		case "state":
			// state may be a UUID string (unexpanded) or an object
			// {"id","name","group"} (expanded). Prefer the name; surface the
			// group as state_group.
			row["state"] = flattenRelationName(value)
			if g := pickField(value, "group"); g != nil {
				row["state_group"] = g
			}
		case "assignees", "labels", "members", "issues", "modules", "favorites":
			// Arrays of UUID strings (unexpanded) or named objects (expanded).
			row[key] = joinRelationList(value)
		case "project", "parent", "cycle", "module", "workspace", "type":
			row[key] = flattenRelationName(value)
		case "member", "created_by", "updated_by", "lead", "default_assignee":
			// People relations: prefer display_name / email when expanded,
			// otherwise the raw id/value.
			row[key] = flattenPersonValue(value)
		default:
...
// flattenRelationName reduces a relation to a readable scalar: the name field
// when the value is an expanded object, otherwise the raw scalar (e.g. a UUID
// string) unchanged.
func flattenRelationName(raw json.RawMessage) any {
	trimmed := strings.TrimSpace(string(raw))
	if trimmed == "" || trimmed == "null" {
		return nil
	}
	if strings.HasPrefix(trimmed, "{") {
		if v := pickField(raw, "name"); v != nil {
			return v
		}
...
// flattenPersonValue reduces a person relation to a readable scalar, preferring
// display_name, then email, then the raw value (commonly a UUID).
func flattenPersonValue(raw json.RawMessage) any {
	trimmed := strings.TrimSpace(string(raw))
	if trimmed == "" || trimmed == "null" {
		return nil
	}
	if strings.HasPrefix(trimmed, "{") {
		for _, field := range []string{"display_name", "email", "first_name", "id"} {
			if v := pickField(raw, field); v != nil {
				return v
			}
		}
		return flattenObject(raw)
...
// joinRelationList joins an array of relations into a comma-separated string.
// Each element is reduced via flattenValue (an object becomes its
// name/display_name; a scalar such as a UUID passes through).
func joinRelationList(raw json.RawMessage) any {
	trimmed := strings.TrimSpace(string(raw))
	if trimmed == "" || trimmed == "null" {
		return nil
	}
...
// field; arrays of named objects (or scalars) are joined.
...
// first available of name/display_name/title/email/id.
func flattenObject(raw json.RawMessage) any {
	var obj map[string]json.RawMessage
	if err := json.Unmarshal(raw, &obj); err != nil {
		return nil
	}
	for _, key := range []string{"name", "display_name", "title", "email", "id"} {
		if v, ok := obj[key]; ok {
...
func stringScalar(raw json.RawMessage) (string, bool) {
	...
	// Numbers and booleans: render their literal form.
	if trimmed[0] != '"' {
