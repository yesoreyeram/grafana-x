...
// Datasource is the Baserow backend data source instance. One instance exists
...
func (d *Datasource) query(ctx context.Context, query backend.DataQuery) backend.DataResponse {
	...
	case "records", "":
		...
		if err != nil {
			log.DefaultLogger.Error("baserow query failed", "refID", query.RefID, "error", err)
	...
	case "count":
		...
		if err != nil {
			log.DefaultLogger.Error("baserow count query failed", "refID", query.RefID, "error", err)
			return backend.ErrDataResponse(backend.StatusInternal, "query failed: "+err.Error())
		}
...
func (d *Datasource) CheckHealth(ctx context.Context, _ *backend.CheckHealthRequest) (*backend.CheckHealthResult, error) {
	if d.settings.BaseURL == "" {
		return &backend.CheckHealthResult{
			...
			Message: "Baserow base URL is not configured",
		}, nil
	}

	if d.settings.AuthMode == AuthPassword {
		if strings.TrimSpace(d.settings.Email) == "" {
			return &backend.CheckHealthResult{
				Status:  backend.HealthStatusError,
				Message: "Baserow email is not configured",
			}, nil
		}
		if strings.TrimSpace(d.settings.password) == "" {
			return &backend.CheckHealthResult{
				Status:  backend.HealthStatusError,
				Message: "Baserow password is not configured (re-enter it; saved secrets are write-only)",
			}, nil
		}
	} else {
		if strings.TrimSpace(d.settings.apiToken) == "" {
			return &backend.CheckHealthResult{
				Status:  backend.HealthStatusError,
				Message: "Baserow API token is not configured (re-enter it; saved secrets are write-only)",
			}, nil
		}
		// Database ID is optional in token mode: the token-check and all-tables
		// endpoints work without it (it only scopes the table list).
	}

	if err := d.client.Ping(ctx); err != nil {
		return &backend.CheckHealthResult{
			Status:  backend.HealthStatusError,
			Message: "Failed to connect to Baserow: " + err.Error(),
		}, nil
	}

	return &backend.CheckHealthResult{
		Status:  backend.HealthStatusOk,
		Message: "Successfully connected to Baserow",
	}, nil
}
...
// handleDatabases serves GET /databases returning every database accessible to
// the user. Only meaningful in the password (JWT) auth mode.
...
// handleTables serves GET /tables[?databaseId=...] returning the list of tables
// in the (configured or supplied) Baserow database.
func (d *Datasource) handleTables(rw http.ResponseWriter, r *http.Request) {
	databaseID := r.URL.Query().Get("databaseId")
	tables, err := d.client.ListTables(r.Context(), databaseID)
	if err != nil {
		writeJSON(rw, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
...
// handleFields serves GET /fields?tableId=... returning the table's fields.
