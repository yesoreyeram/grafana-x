...
// dateKeys lists the ClickUp fields whose values are Unix-millisecond strings
// and should be presented as time columns. ClickUp returns these as strings of
// epoch milliseconds (e.g. "1567780450202").
...
// recordsToFrame converts a slice of flattened ClickUp records (arbitrary JSON
// objects keyed by field name) into a single wide data.Frame conforming to the
// data plane "table" contract.
//
// Row order is preserved exactly as returned by ClickUp, so the query's ordering
...
// specific type that fits every value. Known ClickUp date columns (Unix-millis
...
// ClickUp date columns the value is a Unix-millisecond number/string; otherwise
// it tries the standard ISO/RFC3339 layouts.
func toColumnTime(col string, v any) (time.Time, bool) {
	if v == nil {
		return time.Time{}, false
	}
	if dateKeys[col] {
		if ms, ok := toUnixMillisValue(v); ok {
...
// toUnixMillisValue extracts Unix milliseconds from a value that may be a number
// (float64) or a numeric string.
func toUnixMillisValue(v any) (int64, bool) {
	switch n := v.(type) {
	case float64:
		return int64(n), true
	case json.Number:
...
// timeLayouts lists the formats recognised when inferring non-ClickUp-date time
// columns (e.g. from raw queries returning ISO strings).
...
// ---------------------------------------------------------------------------
// ClickUp task / entity flattening
//
// A ClickUp task is a nested JSON object. flattenTask reduces each nested
// object/array to a scalar (or compact string) so records can flow through the
// same type-inference / frame builder as a flat table. Specific known relations
// are mapped to their most useful representation; other nested values fall back
// to a generic flattener.
// ---------------------------------------------------------------------------

// flattenTask converts a single ClickUp entity object (task, space, folder,
// list, team, ...) into a flat record keyed by field name. Nested relations are
// reduced to readable scalars.
func flattenTask(raw json.RawMessage) map[string]any {
	...
	for key, value := range obj {
		switch key {
		case "status":
			// status: {"status":"in progress","type":"custom",...} -> name + type.
			row["status"] = pickField(value, "status")
			if t := pickField(value, "type"); t != nil {
				row["status_type"] = t
			}
		case "priority":
			// priority may be an object {"priority":"high"} or a bare value/null.
			row["priority"] = flattenPriority(value)
		case "assignees", "watchers", "group_assignees":
			row[key] = joinUserNames(value)
		case "tags":
			row[key] = joinTagNames(value)
		case "creator":
			row[key] = pickField(value, "username")
		case "list", "folder", "project", "space":
...
// flattenPriority reduces a ClickUp priority value to a scalar. Priority is an
// object like {"priority":"high","id":"2"} or null.
func flattenPriority(raw json.RawMessage) any {
	trimmed := strings.TrimSpace(string(raw))
	if trimmed == "" || trimmed == "null" {
		return nil
	}
	if strings.HasPrefix(trimmed, "{") {
		if v := pickField(raw, "priority"); v != nil {
			return v
		}
		return nil
	}
...
// joinUserNames joins an array of user objects into a comma-separated string of
// usernames.
func joinUserNames(raw json.RawMessage) any {
...
// joinTagNames joins an array of tag objects into a comma-separated string of
// tag names.
func joinTagNames(raw json.RawMessage) any {
...
// first available of name/username/title/id.
func flattenObject(raw json.RawMessage) any {
	var obj map[string]json.RawMessage
	if err := json.Unmarshal(raw, &obj); err != nil {
		return nil
	}
	for _, key := range []string{"name", "username", "title", "status", "id"} {
		if v, ok := obj[key]; ok {
			if s, ok := stringScalar(v); ok {
				return s
			}
