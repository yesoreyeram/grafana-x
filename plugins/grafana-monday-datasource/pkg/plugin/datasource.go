...
// Datasource is the monday.com backend data source instance. One instance exists
...
func (d *Datasource) query(ctx context.Context, query backend.DataQuery) backend.DataResponse {
	...
	// Grouped item queries (e.g. "tasks by status", "tasks by owner") are routed
	// to monday.com's server-side `aggregate` query inside ListRecords.
	records, err := d.client.ListRecords(ctx, qm)
	if err != nil {
		log.DefaultLogger.Error("monday.com query failed", "refID", query.RefID, "error", err)
...
func (d *Datasource) CheckHealth(ctx context.Context, _ *backend.CheckHealthRequest) (*backend.CheckHealthResult, error) {
	if d.settings.BaseURL == "" {
		return &backend.CheckHealthResult{
			...
			Message: "monday.com GraphQL URL is not configured",
		}, nil
	}
	if token, _ := d.settings.credential(); strings.TrimSpace(token) == "" {
		return &backend.CheckHealthResult{
			Status:  backend.HealthStatusError,
			Message: "monday.com credential (API token or OAuth token) is not configured",
		}, nil
	}
	...
	if err := d.client.Ping(ctx); err != nil {
		return &backend.CheckHealthResult{
			Status:  backend.HealthStatusError,
			Message: "Failed to connect to monday.com: " + err.Error(),
		}, nil
	}
...
// CallResource routes resource calls (used by the frontend to fetch boards,
// groups, columns and workspaces for the query editor dropdowns).
...
// handleBoards serves GET /boards returning the account boards.
...
// handleGroups serves GET /groups?boardId=...&boardId=... returning the groups
// of the requested boards.
func (d *Datasource) handleGroups(rw http.ResponseWriter, r *http.Request) {
	boardIDs := r.URL.Query()["boardId"]
	groups, err := d.client.ListGroups(r.Context(), boardIDs)
	if err != nil {
		writeJSON(rw, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
	writeJSON(rw, http.StatusOK, map[string]any{"groups": groups})
}

// handleColumns serves GET /columns?boardId=...&boardId=... returning the columns
// of the requested boards.
func (d *Datasource) handleColumns(rw http.ResponseWriter, r *http.Request) {
	boardIDs := r.URL.Query()["boardId"]
	columns, err := d.client.ListColumns(r.Context(), boardIDs)
	if err != nil {
		writeJSON(rw, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
	writeJSON(rw, http.StatusOK, map[string]any{"columns": columns})
}

// handleWorkspaces serves GET /workspaces returning the account workspaces.
