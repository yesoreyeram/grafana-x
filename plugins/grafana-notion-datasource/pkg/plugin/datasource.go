...
// Datasource is the Notion backend data source instance. One instance exists per
...
func (d *Datasource) query(ctx context.Context, query backend.DataQuery) backend.DataResponse {
	...
	case "records", "":
		...
		if err != nil {
			log.DefaultLogger.Error("notion query failed", "refID", query.RefID, "error", err)
	...
	case "count":
		...
		if err != nil {
			log.DefaultLogger.Error("notion count query failed", "refID", query.RefID, "error", err)
			return backend.ErrDataResponse(backend.StatusInternal, "query failed: "+err.Error())
		}
...
func (d *Datasource) CheckHealth(ctx context.Context, _ *backend.CheckHealthRequest) (*backend.CheckHealthResult, error) {
	if d.settings.BaseURL == "" {
		return &backend.CheckHealthResult{
			...
			Message: "Notion base URL is not configured",
		}, nil
	}
	...
	if err := d.client.Ping(ctx); err != nil {
		return &backend.CheckHealthResult{
			Status:  backend.HealthStatusError,
			Message: "Failed to connect to Notion: " + err.Error(),
		}, nil
	}
...
// CallResource routes resource calls (used by the frontend to fetch database
// and property lists).
...
// handleDatabases serves GET /databases returning the list of databases shared
...
// handleProperties serves GET /properties?databaseId=... returning the
...
func (d *Datasource) handleProperties(rw http.ResponseWriter, r *http.Request) {
	...
	properties, err := d.client.ListProperties(r.Context(), databaseID)
	if err != nil {
		writeJSON(rw, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
	writeJSON(rw, http.StatusOK, map[string]any{"properties": properties})
}
